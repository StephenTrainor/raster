...
let add ~x ~y ~value t =
  ...
  let value = Float.round value |> int_of_float in
  let new_pixel = r + value, g + value, b + value in
  set t ~x ~y new_pixel
;;
...
let slice_in_bounds t ~x ~y ~radius =
  ...
  let x_end = min (t.width - 1) (x + radius) in
  let y_end = min (t.height - 1) (y + radius) in
...
let count_differences image_1 image_2 =
  if width image_1 <> width image_2
  then failwith "Width of images do not line up"
  else if height image_1 <> height image_2
  then failwith "Height of images given do not line up";
  foldi image_1 ~init:0 ~f:(fun ~x ~y acc pixel ->
    let other_pixel = get ~x ~y image_2 in
    if not (Pixel.equal pixel other_pixel) then acc + 1 else acc)
;;

let image_comparison_report image_1 image_2 =
  let differences = count_differences image_1 image_2 in
  if differences = 0
  then print_endline "Identical images"
  else
    print_endline
      (Printf.sprintf
         "%s differences were found in the given images"
         (string_of_int differences))
;;
